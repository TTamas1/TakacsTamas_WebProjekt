...
function kategoriak_adatai(selectId){
    const hely = document.getElementById(selectId);
    const alapOpcio = document.createElement('option');
    alapOpcio.value = "";
    alapOpcio.textContent = "-- Összes --";
    hely.appendChild(alapOpcio);

    window.kategoriak.forEach(kategoria => {
...
kategoriak_adatai("kategoriak");

// Teljes lista összefésülése
function frissitLista(){
    const tarolt = JSON.parse(localStorage.getItem("ingatlanok") || "[]");
    return [...ingatlanok, ...tarolt];
}

// Lista megjelenítés
function ingatlanok_adatai(kategoriaId=""){
    const lista = frissitLista();
    const hely = document.getElementById("ingatlanok");
    hely.innerHTML = "";

    const talalatok = lista.filter(i => kategoriaId === "" || i.kategoria_id == kategoriaId);

    if(talalatok.length === 0){
        hely.innerHTML = "<p style='text-align:center; width:100%;'>Nincs találat ebben a kategóriában.</p>";
        return;
    }

    talalatok.forEach(i => {
        // Kategória név kikeresése
        const katNev = window.kategoriak.find(k => k.id == i.kategoria_id)?.nev || "Egyéb";
        
        // Ár formázása (pl: 26 990 000 Ft)
        ...
        kartya.innerHTML = `
            <img src="${i.kepUrl}" alt="${katNev}" onerror="this.src='https://via.placeholder.com/300x200?text=Nincs+Kép'">
            <div class="card-body">
                ...
                <p class="card-info">
                    <span class="tag">${i.hirdetesDatuma}</span>
                    ${i.tehermentes ? '<span class="tag tehermentes">Tehermentes</span>' : ''}
                </p>
                <p>${i.leiras.substring(0, 150)}</p>
            </div>
        `;
...
document.getElementById('valasszForm').addEventListener('submit', function(event){
    ...
    const katId = document.getElementById('kategoriak').value;
