...
function kategoriak_adatai_feladas(){
    const select = document.getElementById("kategoriakFelad")
...
document.getElementById('feladForm').addEventListener('submit', function(event){
    ...
    let adatok = Object.fromEntries(new FormData(this))
    adatok.id = (JSON.parse(localStorage.getItem("ingatlanok") || "[]").length || 0) + 1
    adatok.tehermentes = adatok.tehermentes === "on" ? 1 : 0  // ha checkbox
    adatok.ar = parseInt(adatok.ar)
    adatok.kategoria_id = parseInt(adatok.kategoriak_id);
    delete adatok.kategoriak_id;
    
    let lista = JSON.parse(localStorage.getItem("ingatlanok") || "[]")

    adatok.id = lista.length + 1 

    lista.push(adatok)

    localStorage.setItem("ingatlanok", JSON.stringify(lista))
